...
截至 2026-07-17，Task 11.0 至 Task 12.4 已完成并通过各自的自动化测试、规格复核与质量复核。界面默认语言固定为简体中文；Vite 双服务浏览器验收与 FastAPI 单服务演示验收均已通过。当前唯一未完成的 Task 12 验收项是真实 Neo4j 问答 smoke：本机 `localhost:7687` 未启动，因此浏览器主流程使用严格契约 fixture 验证，未将其记为 live 数据库联调通过。
